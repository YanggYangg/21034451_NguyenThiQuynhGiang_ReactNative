...
const API_Screen02 = () => {
  ...
  const { userName } = route.params || {};  //check và lay userName tu param
  ...
  useEffect(() => {
    if (route.params?.newJob) {
      ...
      setData((prevData) => [newJob, ...prevData]); //update ds them vao dau ds
    }
  }, [route.params?.newJob]);
  ...
  const renderItem = ({ item }) => (
    <View style={styles.itemContainer}>
      ...
      <TouchableOpacity 
      onPress={() => navigation.navigate('API_Screen04', { //Screen04 edit
        userName, 
        job: item.title, 
        jobId: item.id, 
      })}
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity 
        ...
        onPress={() => navigation.navigate('API_Screen03', { userName })}  // Điều hướng sang màn hình API_Screen03
      >
